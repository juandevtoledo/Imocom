...
public class ClientesDetalleFacesBean extends AbstractFacesBean implements Serializable {
    ...
    private String textoHabeasDataCliente, colorTextoHabeasDataCliente;
    ...
    protected void build() {
        ...
        try {
            ...
            //Se busca el indicativo
            Tipo indicativoCliente = iTipoService.findByTipoNombreEtiqueta("INDICATIVO", detalleCliente.getDepartamento());
            Tipo indicativoClientePais = iTipoService.findByTipoNombreEtiqueta("INDICATIVO_PAIS", "COLOMBIA");
            String indicativo = indicativoClientePais.getTipoValor() + indicativoCliente.getTipoValor();
            logger.info("** Indicativo ** " + indicativo + " " + _clienteConsultado.getIndicativo());
            if (_clienteConsultado.getTelefonoPpal().length() < 10) {
                detalleCliente.setTelefono(indicativo + _clienteConsultado.getTelefonoPpal());
            } else {
            ...
            if (detalleCliente == null || detalleCliente.getListaContactos() == null || detalleCliente.getListaContactos().isEmpty()) {
                textoHabeasDataCliente = "NINGUNO";
            } else {

                //List<DocumentosClienteDTO> listaDocumentosClienteHabeasDataDTO = iRidcConnection.searchDocsClienteHabeasData(detalleCliente.getNit());
                textoHabeasDataCliente = "NINGUNO";
                int cantidadListaContacto = detalleCliente.getListaContactos().size();
                int cantidadContactosHabeasData = 0;
                //LISTA DE CONTACTOS
                for (ContactoCliente unContactoCliente : detalleCliente.getListaContactos()) {

                    String nombre = GerenerNombreUnicos.getNombre( unContactoCliente.getNombre());
                    
                    if (nombre == null) {
                        continue;
                    }

                    List<HabeasDataContacto> listaHabeas = iHabeasDataContactoEBSLocal.findAllByIdentificacionYNit( nombre, detalleCliente.getNit() );

                    cantidadContactosHabeasData += (listaHabeas.isEmpty() ? 0 : 1);

                }
                if( cantidadContactosHabeasData > 0 ){
                    textoHabeasDataCliente = (cantidadContactosHabeasData >= cantidadListaContacto) ? "TODOS" : "ALGUNOS";
                }
            }
                ...
                if (response != null) {
                    ...
                    if (contactosResponse != null && !contactosResponse.isEmpty()) {
                        ...
                        for (ContactoVO contactoItem : contactosResponse) {
                            String telefono = contactoItem.getTelefono();
                            //Se busca el indicativo
                            Tipo indicativoCliente;
                            try {
                                // buscando indicativo del cliente
                                logger.info("Buscando indicativo del Cliente ");
                                indicativoCliente = iTipoService.findByTipoNombreEtiqueta("INDICATIVO", detalleCliente.getDepartamento());
                                Tipo indicativoClientePais = iTipoService.findByTipoNombreEtiqueta("INDICATIVO_PAIS", "COLOMBIA");
                                String indicativo = indicativoClientePais.getTipoValor() + indicativoCliente.getTipoValor();
                                telefono = indicativo+telefono;
                            } catch (ServiceException ex) {
                                logger.error("Error obteniendo Telefonos "+ex.getMessage(),ex);
                            } catch (Exception ex) {
                                logger.error("Error obteniendo Telefonos "+ex.getMessage(),ex);
                            }
                            
                            contactoItem.setTelefono(telefono);
    ...
    public String getTextoHabeasDataCliente() {
        return textoHabeasDataCliente;
    }

    public void setTextoHabeasDataCliente(String textoHabeasDataCliente) {
        this.textoHabeasDataCliente = textoHabeasDataCliente;
    }

    public String getColorTextoHabeasDataCliente() {
        colorTextoHabeasDataCliente = "red";
        if ("TODOS".equals(textoHabeasDataCliente)) {
            colorTextoHabeasDataCliente = "green";
        } else if ("ALGUNOS".equals(textoHabeasDataCliente)) {
            colorTextoHabeasDataCliente = "yellow";
        }
        return colorTextoHabeasDataCliente;
    }

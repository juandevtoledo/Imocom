...
@ManagedBean
@ViewScoped
public class EventoModificarFacesBean extends AbstractFacesBean implements Serializable {

    private static final Logger logger = Logger.getLogger(EventoModificarFacesBean.class);
    ...
    protected void build() {
        ...
        try {
            ...
            consultaNombreNitCliente(Long.toString(evento.getIdcliente()));
            ...
            for(InvitadoVisita invitadoTmp : evento.getInvitadoVisitaSet()){
                listaUsuariosSeleccionados.add(invitadoTmp.getIdUsuario());
            }
            
            System.out.println("evento.getFechainicio() " + evento.getFechainicio());
            System.out.println("evento.getFechafinal() " + evento.getFechafinal());
            horaIni = DateUtil.formatTime24h(evento.getFechainicio());
            horaFin = DateUtil.formatTime24h(evento.getFechafinal());
    ...
    public void modificarAction(ActionEvent actionEvent) {
        ...
        if (DateUtil.validarHorasVisita(horaIni, horaFin)) {
            if (nit != null && nit.trim().length() > 0) {
                ...
                try {
                    ...
                    evento = iEventoService.update(evento);
                    try{
    ...
    private void consultaNombreNitCliente(String idCliente) {
        ...
        try {
            ...
            if (response != null && response.getClientes() != null) {
               ...
               nombreCliente =  response.getClientes().get(0).getNombreCliente();
               nit =  response.getClientes().get(0).getNitCliente();
            }
